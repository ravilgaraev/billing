...
<div class="logo_sf">
    ...
    <table>
        ...
        <?php if('Usd' == $user['paymentmethod'] || 'Euro' == $user['paymentmethod']) :?>
        <tr>
            <td class="odin text-right">SWIFT</td>
            <td class="dva">NBFAUZ2X</td>
            <td class="tri"></td>
            <td class="odin text-right">SWIFT</td>
            <td class="dva"><?php echo $user['swift'];?></td>
        </tr>
        <?php endif; ?>        
        <tr>
    ...
    <div class="row">
        <table style="width: 100%" class=" table table-print">
            ...
            <tr>
                ...
                <td>
                    <?php echo number_format($many*100/($stavkands+100),2,'.',','); ?>
                </td>
                <!--<td colspan="2" class="text-center">Без АН</td>-->
                <td class="text-right"><?php echo ('n' == $user["nds"])?"Без НДС":$stavkands; ?></td>
                <td class="text-right">
                    <?php echo ('n' == $user["nds"])?"Без НДС":number_format($many - $many*100/($stavkands+100),2,'.',','); ?>
                </td>
                <td class="text-right"><?php echo number_format($many,2,'.',','); ?></td>
            </tr>
            <tr>
                <td>Всего к оплате<?php echo ('n' == $user["nds"])?" Без НДС":" с учетом НДС"; ?></td>
...
//                            if("Usd" == $user['paymentmethod']){$valuta=2;}else{$valuta=1;}
                        ...
                        {
                            if("Usd" == $user['paymentmethod']){$valuta=2;}else{$valuta=0;}
                            $sum = Model::factory('sumpropis')->num2stren($many,$valuta);
                        }
                        list($sum[0], $sum[1]) = mb_strtoupper($sum[0].$sum[1], 'UTF8');
                        echo $sum,('n' == $user["nds"])?" Без НДС ":", включая НДС ".number_format($many - $many*100/($stavkands+100),2,'.',',');
                    ?>
